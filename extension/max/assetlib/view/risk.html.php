    ...
    <table class="table has-sort-head" id='riskList'>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class='text-center' id='pri'><?php echo "<span class='pri-{$risk->pri}'>" . zget($lang->risk->priList, $risk->pri) . "</span>";?></td>
          ...
          <?php endif;?>
          <td title="<?php echo zget($users, $risk->createdBy);?>"><?php echo zget($users, $risk->createdBy);?></td>
          <td><?php echo helper::isZeroDate($risk->createdDate) ? '' : substr($risk->createdDate, 0, 11);?></td>
          <?php if($browseType == 'all' or $browseType == 'bysearch'):?>
          <td class='c-assignedTo'><?php echo $risk->status == 'active' ? zget($users, $risk->assignedTo) : '';?></td>
