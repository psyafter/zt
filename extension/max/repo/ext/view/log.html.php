...
<div id="mainContent">
  <nav id="contentNav">
    <ul class="nav nav-default">
      ...
      <?php if($info->kind == 'file') echo '<li>' . html::a($this->repo->createLink('blame', "repoID=$repoID&objectID=$objectID&entry=$encodeEntry&revision=$revision"), $lang->repo->blame, '', "data-app='{$app->tab}'") . '</li>';?>
    ...
    <table class='table table-fixed' id='logList'>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class='versions'><?php echo html::a($this->repo->createLink('revision', "repoID=$repoID&revision=" . $log->revision), substr($log->revision, 0, 10), '', "data-app='{$app->tab}'");?></td>
