...
class imMessage extends model
{
    ...
    public function create($messageList = array(), $userID = 0)
    {
        ...
        foreach($messageList as $message)
        {
            ...
            $chatMessageID[$message->cgid] = $message->id;
        }
        ...
        foreach(array_keys($chatMessageID) as $cgid)
        {
            $this->dao->update(TABLE_IM_CHAT)
                ->set('lastActiveTime')->eq($now)
                ->beginIf(isset($chatMessageID[$cgid]))->set('lastMessage')->eq($chatMessageID[$cgid])->fi()
                ->set('lastMessageIndex')->eq($chatMessageIndex[$cgid])
                ->where('gid')->eq($cgid)
                ->exec();
        }
