      ...
      <small class='dropdown versions'>
        <a href='#' data-toggle='dropdown' class='text-muted'><?php echo '#' . $oldVersion;?> <span class='caret'></span></a>
          ...
          for($i =$newDoc->version; $i > 0; $i--)
          {
              ...
              echo '<li' . $class .'>' . html::a(inlink('diff', "objectType=$objectType&docID=$newDoc->id&newVersion=$newVersion&oldVersion=$i"), '#' . $i) . '</li>';
          }
          ?>
        </ul>
      </small>
      ...
      <small class='dropdown versions'>
        <a href='#' data-toggle='dropdown' class='text-muted'><?php echo '#' . $newVersion;?> <span class='caret'></span></a>
          ...
          for($i =$newDoc->version; $i > 0; $i--)
          {
              ...
              echo '<li' . $class .'>' . html::a(inlink('diff', "objectType=$objectType&docID=$newDoc->id&newVersion=$i&oldVersion=$oldVersion"), '#' . $i) . '</li>';
          }
          ?>
        </ul>
      </small>
